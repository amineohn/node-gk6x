...
yargs(hideBin(process.argv))
  .command(
    "config <file>",
    "Use le or pixeltris config format (easier to use) to setup your keyboard. It guesses format based on file-extension.",
    (y) => {},
    async ({ file }) => {
  ...
  .command(
    ...
    async ({ file }: { file: string }) => {
      ...
      if (isLE) {
        ...
        await Promise.all(
          Object.keys(out).map((f) =>
            writeFile(`${name}-${f}.le`, out[f], "binary")
          )
        );
        console.log(
          `Files saved in ${Object.keys(out)
            .map((f) => `${name}-${f}.le`)
            .join(", ")}`
        );
      }
  ...
  .command(
    "apple",
    "Force your keyboard to pretend it's an official apple keyboard.",
    (y) => {},
    (a) => {
      try {
  ...
  .example(
    "$0 config mycoolfile.le",
    "Load config from mycoolfile.le onto keyboard"
  )
  .example(
    "$0 config mycoolfile.txt",
    "Load config from mycoolfile.txt onto keyboard"
  )
  .example(
    "$0 convert mycoolfile.le",
    "Convert mycoolfile.le to pixeltris config format"
  )
  .example("$0 convert mycoolfile.txt", "Convert mycoolfile.txt to le format")
  .example(
    "$0 apple",
    "Make your keyboard appear to be an official apple keybaord"
  )
  .example(
    "$0 normal",
    "Make your keyboard work right again (after running apple command)"
  )
